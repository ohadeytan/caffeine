 ...
 */
package com.github.benmanes.caffeine.cache.simulator.parser.systor;
...
public final class SystorTraceReader extends TextTraceReader {
  ...
  public SystorTraceReader(List<String> filePaths) {
    super(filePaths);
  }

  @Override
  public LongStream events() throws IOException {
    return lines().skip(1).flatMapToLong(line -> {
      String[] array = line.split(",", 6);
      if (array.length <= 5) {
          return LongStream.empty();
      }
      char readWrite = Character.toLowerCase(array[2].charAt(0));
      if (readWrite != 'r') {
    	  return LongStream.empty(); 
      }
